...
export const cehModules: Module[] = [
  ...
  },
  {
    ...
    description: 'OSINT techniques, passive and active reconnaissance, and footprinting methodologies.',
    examWeight: '8%',
    ...
    topics: [
      {
        ...
        content: 'Footprinting is the process of collecting as much information as possible about a target organization. It can be passive (without touching target systems) or active (directly engaging with targets).',
        keyPoints: [
          'Passive Footprinting: No direct interaction with target',
          'Active Footprinting: Direct interaction (scanning, probing)',
          'Information types: Network, System, Organization',
        ],
      },
      {
        id: 'm02-t02',
        title: 'WHOIS and DNS Reconnaissance',
        content: 'WHOIS provides domain registration information. DNS reconnaissance reveals name servers, mail servers, and subdomains.',
        commands: [
          { command: 'whois example.com', description: 'Get domain registration info' },
          { command: 'nslookup example.com', description: 'Query DNS records' },
          { command: 'dig example.com ANY', description: 'Get all DNS records' },
          { command: 'dig @8.8.8.8 example.com MX', description: 'Query mail servers' },
          { command: 'dig axfr @ns1.example.com example.com', description: 'Attempt zone transfer' },
        ],
      },
      {
        id: 'm02-t03',
        title: 'Google Hacking (Dorks)',
        content: 'Google dorks use advanced search operators to find sensitive information indexed by search engines.',
        commands: [
          { command: 'site:example.com filetype:pdf', description: 'Find PDFs on domain' },
          { command: 'inurl:admin site:example.com', description: 'Find admin pages' },
          { command: 'intitle:"index of" site:example.com', description: 'Find directory listings' },
          { command: '"@example.com" email', description: 'Find email addresses' },
          { command: 'site:example.com ext:sql OR ext:env OR ext:config', description: 'Find sensitive files' },
        ],
      },
      {
        id: 'm02-t04',
        title: 'Network Reconnaissance',
        content: 'Network reconnaissance discovers the target network infrastructure, IP ranges, and connectivity.',
        commands: [
          { command: 'traceroute example.com', description: 'Trace network path' },
          { command: 'ping example.com', description: 'Check host availability' },
          { command: 'curl -I http://example.com', description: 'Get HTTP headers' },
        ],
      },
      {
        id: 'm02-t05',
        title: 'OSINT Tools',
        content: 'Open Source Intelligence tools automate the collection of publicly available information.',
        commands: [
          { command: 'theHarvester -d example.com -b all', description: 'Email and subdomain harvesting' },
          { command: 'maltego', description: 'Visual link analysis' },
          { command: 'recon-ng', description: 'OSINT framework' },
          { command: 'shodan search "apache country:US"', description: 'Shodan device search' },
        ],
      },
    ],
    keyTools: ['theHarvester', 'Maltego', 'Recon-ng', 'Shodan', 'WHOIS', 'dig', 'nslookup'],
    countermeasures: [
      'Limit information in public WHOIS records',
      'Use privacy protection services',
      'Regularly audit publicly available information',
  ...
  },
  {
    ...
    description: 'Network scanning techniques, port scanning types, OS fingerprinting, and IDS evasion.',
    examWeight: '8%',
    estimatedQuestions: 10,
    duration: '3h 45m',
    topics: [
      {
        ...
        content: 'Host discovery identifies live systems on a network before port scanning.',
        commands: [
          { command: 'nmap -sn 192.168.1.0/24', description: 'Ping sweep (no port scan)' },
          { command: 'nmap -PR 192.168.1.0/24', description: 'ARP ping scan' },
          { command: 'nmap -PU 192.168.1.0/24', description: 'UDP ping scan' },
        ],
      },
      {
        id: 'm03-t02',
        title: 'Port Scanning Types',
        content: 'Different TCP scan types have different characteristics for stealth and effectiveness.',
        commands: [
          { command: 'nmap -sT 192.168.1.1', description: 'TCP Connect scan (full handshake)' },
          { command: 'nmap -sS 192.168.1.1', description: 'SYN scan (stealth, half-open)' },
          { command: 'nmap -sU 192.168.1.1', description: 'UDP scan' },
          { command: 'nmap -sF 192.168.1.1', description: 'FIN scan' },
          { command: 'nmap -sX 192.168.1.1', description: 'XMAS scan (FIN+URG+PUSH)' },
          { command: 'nmap -sN 192.168.1.1', description: 'NULL scan' },
          { command: 'nmap -sA 192.168.1.1', description: 'ACK scan (firewall detection)' },
        ],
      },
      {
        id: 'm03-t03',
        title: 'TCP Flags Reference',
        content: 'Understanding TCP flags is essential for advanced scanning and firewall evasion.',
        keyPoints: [
      ...
      {
        id: 'm03-t04',
        title: 'Version and OS Detection',
        content: 'Service version detection and OS fingerprinting provide valuable target information.',
        commands: [
          { command: 'nmap -sV 192.168.1.1', description: 'Service version detection' },
          { command: 'nmap -O 192.168.1.1', description: 'OS detection' },
          { command: 'nmap -A 192.168.1.1', description: 'Aggressive scan (all features)' },
        ],
      },
      {
        id: 'm03-t05',
        title: 'IDS Evasion Techniques',
        content: 'Various techniques can help evade intrusion detection systems during scanning.',
        commands: [
          { command: 'nmap -f 192.168.1.1', description: 'Fragment packets' },
          { command: 'nmap -D RND:10,ME 192.168.1.1', description: 'Use decoys' },
          { command: 'nmap --source-port 53 192.168.1.1', description: 'Spoof source port' },
          { command: 'nmap -T0 192.168.1.1', description: 'Paranoid timing (slowest)' },
          { command: 'nmap --data-length 25 192.168.1.1', description: 'Append random data' },
        ],
      },
    ],
    keyTools: ['Nmap', 'Masscan', 'Unicornscan', 'Hping3', 'Zmap'],
    countermeasures: [
      'Implement network segmentation',
      'Use IDS/IPS systems',
      'Configure firewalls to limit scan exposure',
    ...
    commonPorts: [
      ...
      { port: 23, protocol: 'TCP', service: 'Telnet', description: 'Unencrypted remote access' },
      { port: 25, protocol: 'TCP', service: 'SMTP', description: 'Email sending' },
      { port: 53, protocol: 'UDP/TCP', service: 'DNS', description: 'Domain Name System' },
      { port: 80, protocol: 'TCP', service: 'HTTP', description: 'Web traffic' },
      { port: 110, protocol: 'TCP', service: 'POP3', description: 'Email retrieval' },
      { port: 143, protocol: 'TCP', service: 'IMAP', description: 'Email sync' },
      { port: 443, protocol: 'TCP', service: 'HTTPS', description: 'Encrypted web' },
